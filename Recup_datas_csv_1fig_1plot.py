...
with open(file1, 'r') as read_obj:
    ...
    for row in csv_reader:
        ...
        if (not math.isinf(float(row[1])) or not math.isnan(float(row[1]))
            ...
            time1.append(dt1.timestamp()-7181-1718189155)                               # on récupère la date exprimée en secondes
...
line1_2, = ax.plot(time1, ch1_2, label='Lower Mosfet (Fiber Optic)', color='b', linestyle='-')     
#line1_3, = ax.plot(time1, ch1_3, label='Transformer Copper', color='g', linestyle='-')
line1_4, = ax.plot(time1, ch1_4, label='Transformer Ferrite (Fiber Optic)', color='y', linestyle='-')
line1_5, = ax.plot(time1, ch1_5, dashes=[6, 2], label='Ambient (Fiber Optic)', color='g')
...
line2_1, = ax.plot(time2, ch2_1, dashes=[5, 8], label='Higher Mosfet (Box camera)', color='r')   
line2_2, = ax.plot(time2, ch2_2, dashes=[5, 8], label='Lower Mosfet (Box camera)', color='b')   
